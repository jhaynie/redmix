var path = require('path'),
	...
	configDir = path.join(process.env.HOME || process.env.USERPROFILE,'.appcelerator'),
...
function getACS (opts) {
	...
	if (!config.username || !config.session || !config.key) {
		fail("Not connected. Run redmix connect");
	}
	var acsApp = new ACSNode(config.key,{
		prettyJson: true, 
		autoSessionManagement: false
	});
	acsApp.sessionCookieString = '_session_id='+config.session;
	return acsApp;
}
